...
from keras.layers import Dense, Input, Concatenate, BatchNormalization, Dot, Add, LayerNormalization, Dropout, Conv1D, AveragePooling1D, Flatten, Layer
from keras.losses import BinaryCrossentropy, Loss, losses_utils, CosineSimilarity
from keras.metrics import BinaryAccuracy, Precision, Recall, Mean
...
class EmbeddingComparator(Model):
...
def embedding_comparator_model(input_shape, margin=0.5):
  code_input = Input(
    shape=input_shape,
    name="code_embedding",
  )
  comment_positive_input = Input(
    shape=input_shape,
    name="comment_positive_embedding",
  )
  comment_negative_input = Input(
    shape=input_shape,
  ...
  distance_layer = DistanceLayer()(
    code_input,
    comment_positive_input,
    comment_negative_input,
  )

  model = Model(inputs=[code_input, comment_positive_input, comment_negative_input], outputs=distance_layer)
  comp = EmbeddingComparator(model, margin=margin)
  comp.compile(
    optimizer=Adam(0.01),
