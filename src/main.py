...
train_samples = 10
model_path = f'models/{hidden_layer_activation}_{output_activation}_dropout_{train_samples}'
...
if mode == 'valid':
  ...
  def training_dataset():
    for pair_doc in pairs.find().limit(int(train_samples / 2)):
      code_emb, comment_emb, target = generate_code_embeddings(
          pair_doc['code_tokens']), generate_comment_embeddings(pair_doc['comment_tokens']), tf.ones(shape=(batch_size, ))
      yield ((code_emb, comment_emb), target)

      negative_doc = random_pair(exclude_id=pair_doc['_id'])
      negative_code_emb, negative_comment_emb, negative_target = generate_code_embeddings(
          pair_doc['code_tokens']), generate_comment_embeddings(negative_doc['comment_tokens']), tf.zeros(shape=(batch_size, ))
