...
class EmbeddingGenerator:
    ...
    def generate_target(self, batch_pairs: List[CodeCommentPair]) -> tf.Tensor:
        target_embedding = tf.convert_to_tensor(
            [pair.target for pair in batch_pairs])
    ...
    def concat_embeddings(self, text_embedding: tf.Tensor, code_embedding: tf.Tensor) -> tf.Tensor:
    ...
    def _get_embeddings(self, data: str | list[str], tokenizer: PreTrainedTokenizer | PreTrainedTokenizerFast, model: EmbeddingModel) -> tf.Tensor:
        input = tokenizer(data, return_tensors="tf",
